...
- **设置页搜索要搜到页面里的行，命中带路径、跳转后高亮**：左导航搜索框（`src/components/application/settings/settings-shell.tsx`）分两条 lane：「命中行」lane 按目标页分组（组标题=页面名，行内两行是「行标签 + 所在卡片」，连起来读就是路径 `通用 › 桌面宠物 › 角色`），点选或回车打开该页、滚动到那一行并给行画 1.2s 内侧 focus ring（`settings-rows.tsx` 的 `SettingsRow anchor` + flash context）；「命中页面」lane 保留原来的页标题子串过滤，两条 lane 都空才显示「没有匹配的设置」。行索引是**声明式**的（`src/features/settings/settings-search.ts` 的注册表 + 每页一份清单，通用页在 `general-search.ts`，跟着页面在 `sections.tsx` 一起注册）——不预渲染页面扫 DOM：`GeneralSection` 挂载就读设置、拉宠物列表，插件页是任意代码，隐藏挂载会带来真实副作用。匹配是「行标签 / 所在卡片 / 别名（`keywords`）」的大小写不敏感子串（卡片名可搜是有意的：`外观`、`桌面宠物` 是用户记得的名字，命中行的第二行正好解释了为什么命中），结果行标签里的 i18next 计数占位符会被去掉（`管理历史记录 ({{count}})` → `管理历史记录`）。页面往往是异步画出行的（通用要等设置读回），所以跳转后要等锚点出现再滚动（MutationObserver + 3s 放弃），高亮由行自己按当次锚点渲染，不会「跳到空处」；`prefers-reduced-motion` 下滚动改为瞬时。回归：`settings-search.test.ts`、`general-search.test.tsx`、`settings-shell.test.tsx`。
...
| 版本 | 时间 | 内容 |
|---|---|---|
| v0.54 | 2026-09-24 | 设置搜索可搜页面内部的行：左导航搜索分两条 lane（「命中行」按页面分组、行内显示「标签 + 所在卡片」，点选/回车打开页面并滚动到该行、1.2s 内侧 focus ring 高亮；「命中页面」仍是页标题子串过滤），行索引为声明式（`settings-search.ts` + 每页清单如 `general-search.ts`），通用页全量覆盖；§3 补充规则 |
